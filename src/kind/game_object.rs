...
#[async_trait]
impl Resource for GameObjectResource {
  ...
  async fn input_files(&self) -> Result<Vec<PathBuf>> {
    Ok(vec![self.get_object()])
  }

  async fn output_files(&self) -> Result<HashMap<String, Vec<u8>>> {
    Ok(HashMap::from([(
      "object.kdl".to_owned(),
      fs::read(self.get_object()).await.unwrap(),
    )]))
  }
}
...
impl GameObjectResource {
  pub fn get_object(&self) -> PathBuf {
    self
      ...
      .unwrap_or_else(|| self.get_root().join("object.kdl"))
  }
}
